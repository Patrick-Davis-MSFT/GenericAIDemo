...
export default function Summarize() {
  ...
  const onChangeSentenceCount = (value: number | undefined) =>
    setSentenceCount(value ?? 20);
  const onSummaryTypeChange = (
    _ev: React.MouseEvent<HTMLElement>,
    checked?: boolean
  ) => setUseAbstractive(checked ?? false);
  return (
    <div>
      <Stack>
        ...
        <Stack tokens={{ childrenGap: 10, padding: 5 }}>
          ...
          <Stack.Item>
            <Text>
              ...
              <br /> Abstractive summarization: Generates a summary that may not
              use the same words as those in the document, but captures the main
              idea.
            </Text>
            <Toggle
          ...
          <Stack.Item>
            ...
            <Slider
              ...
              disabled={useAbstractive}
              value={sentenceCount}
              min={1}
              max={20}
              step={1}
              defaultValue={20}
              onChange={onChangeSentenceCount}
              showValue
              snapToStep
            />
          </Stack.Item>
